<template>
  <div class="preview-code">
    <textarea style="opacity: 0;height: 0;padding: 0;position: absolute;z-index: -1" name="" ref="code" cols="30" rows="10" v-model="code"></textarea>
    <div class="preview">
      <slot></slot>
    </div>
    <div class="code" :style="{height: height, opacity: showCode ? 1 : 0, marginTop: showCode ? '24px' : 0}">
    ...
    methods: {
      ...
      initHeight() {
        ...
        if (this.$refs.description) {
          ['paddingTop', 'paddingBottom', 'marginTop', 'marginBottom', 'height'].forEach(item => {
            desPaddingMargin += this.declatationConvertNumber(this.$refs.description, item)
          });
        }
        let codeBox = 0;
        ['paddingTop', 'paddingBottom', 'marginTop', 'marginBottom', 'height'].forEach(item => {
