<template>
  <div style="height: 100vh;overflow: scroll">
    ...
    <div style="margin-top: 300px;display: flex;justify-content: space-around">
      <ve-button v-tip.left="{VNode: renderColor, effect: 'light', target: 'click'}">删除Left Button</ve-button>
      <ve-button v-if="leftButton" v-tip.top="{html: '<h1>hello</h1>', effect: 'light', offset: 30}">top</ve-button>
      ...
      <ve-button v-if="rightButton" v-tip="{placement: 'right', content: 'hello', effect: 'light'}">Right</ve-button>
      <ve-button v-tip.bottom="{VNode: renderColor, effect: 'light', target: 'click'}">Bottom</ve-button>
      <ve-button v-tip.right="{VNode: renderColor, effect: 'light', target: 'click'}">right</ve-button>
    </div>
  </div>
</template>
