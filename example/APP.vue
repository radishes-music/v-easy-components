<template>
  <div style="position:relative;width: 800px;margin: 0 auto">
    <h1 v-loading-dom="visible.title1" data-node-num="3" class="title">{{ titlea }}</h1>
    <div v-loading-dom="visible.content1" data-node-num="30" class="main">
      <p v-for="(item, index) in data1" :key="index">{{ item }}</p>
    </div>
    <hr class="hr" />
    <h1 v-loading-dom="visible.title2" data-node-num="2" class="title">{{ titleb }}</h1>
    <div v-loading-dom="visible.content2" data-node-num="120" class="main">
...
<script>
  export default {
    ...
    methods: {
      ...
      async task() {
        this.titlea = await this.promise(600, '诗歌 - 李白');
        ...
        this.titleb = await this.promise(1200, '诗歌 - 杜甫');
